...
async function drawWebcamContinuous(barcodeDetector){
    let bcstat = document.getElementById('bcstatus');
    ctx.drawImage(video,0,0);
    const barcodes = await barcodeDetector.detect(video)
    if (barcodes.length > 0) {
        let newVal = barcodes.map(barcode => barcode.rawValue)
        if (bcstat.innerHTML != newVal) {
            beep()
            bcstat.innerHTML = newVal
            fetchBook(newVal)
        }
    }
    requestAnimationFrame(() => {drawWebcamContinuous(barcodeDetector)});
}
